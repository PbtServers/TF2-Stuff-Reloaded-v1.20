...
public class TF2ActionHandler implements TF2MessageHandler<TF2Message.ActionMessage, TF2Packet> {

	/*
	 * public static Map<LivingEntity,Integer> playerAction=new
	 * HashMap<LivingEntity,Integer>(); public static
	 * Map<LivingEntity,Integer> playerActionClient=new
	 * HashMap<LivingEntity,Integer>();
	 */
	// public static ThreadLocalMap<LivingEntity,Integer> playerAction=new
	// ThreadLocalMap<LivingEntity,Integer>();
	// public static ThreadLocalMap<LivingEntity,Integer>
	// previousPlayerAction=new ThreadLocalMap<LivingEntity,Integer>();
	@Override
	public TF2Packet onMessage(final TF2Message.ActionMessage message, final MessageContext ctx) {
		if (ctx.side == Dist.SERVER) {
			...
			((ServerLevel) player.world).addScheduledTask(() -> {
				if (message.value <= 15) {
					...
					if (wearer == null || !(wearer instanceof Player))
						wearer = player;
					...
					player.closeScreen();
					final MerchantOffers listg = player.world.getCapability(TF2weapons.WORLD_CAP, null).lostItems
							.get(player.getName());
					if (listg != null) {
						Iterator<MerchantOffer> iterator = listg.iterator();
						while (iterator.hasNext()) {
							MerchantOffer recipe = iterator.next();
					...
					player.displayVillagerTradeGui(new Merchant() {
						...
						public void setCustomer(Player player) {
							if (player == null && list != null) {
								Iterator<MerchantOffer> iterator = list.iterator();
								while (iterator.hasNext()) {
									MerchantOffer recipe = iterator.next();
						...
						public ITextComponent getDisplayName() {
							return new Component("gui.recoveritems");
						}
					...
					// TF2GuiOpener.openGui(player, TF2weapons.instance, 4, player.world,(int)
					...
					if (!WeaponsCapability.get(player).isGrappled()) {
						...
						if (player.getHeldItemMainhand().getItem() instanceof ItemGrapplingHook) {
							WeaponsCapability.get(player).setPrimaryCooldown(InteractionHand.MAIN_HAND,
					...
					if (player != null) {
						if (id5 == 0) {
							...
							if (!success1) {
								...
								while (it1.hasNext()) {
									...
									player.world.getChunkFromBlockCoords(pos1).getEntitiesOfTypeWithinAABB(
											EntityMedic.class, new AABB(pos1), list1,
							...
							if (!success2) {
								...
								while (it2.hasNext()) {
									...
									player.world.getChunkFromBlockCoords(pos2).getEntitiesOfTypeWithinAABB(
											EntityTF2Character.class, new AABB(pos2), list2,
							...
							List<Mob> attackers = player.world.getEntitiesWithinAABB(Mob.class,
							...
							HitResult trace = player.world.rayTraceBlocks(player.getPositionEyes(1),
			...
			final LivingEntity player = ClientHandler.getClientEntity(message.entity);
			Minecraft.getMinecraft().addScheduledTask(() -> {
				if (message.value <= 15)
					...
					if (player != null)
						TF2PlayerCapability.get((Player) player).setEquipBackpackItem(true);
				} else if (message.value == 32) {
					if (player != null)
						TF2PlayerCapability.get((Player) player).setEquipBackpackItem(false);
				}
			});
		}
		return null;
	 ...
	 * TF2MessageHandler<TF2Message.ActionMessage, TF2Packet> {
	 *
	 * @Override public TF2Packet onMessage(TF2Message.ActionMessage message,
	 * MessageContext ctx) { LivingEntity player=(LivingEntity)
	 * Minecraft.getMinecraft().theWorld.getEntityByID(message.entity);
	 * handleMessage(message, player); return null; }
	 *
	 * }
	 */
	public static void handleMessage(TF2Message.ActionMessage message, LivingEntity player, boolean client) {
		if (player != null) {
			/*
			 * int oldValue=playerAction.get().containsKey(player)?playerAction.get(
			 * ).get(player):0; if(player.getHeldItem(InteractionHand.MAIN_HAND) != null &&
			 * player.getHeldItem(InteractionHand.MAIN_HAND).getItem() instanceof ItemUsable){
			 * if((oldValue&1)==0&&(message.value&1)!=0){
			 * ((ItemUsable)player.getHeldItem(InteractionHand.MAIN_HAND).getItem()).
			 * startUse(player.getHeldItem(InteractionHand.MAIN_HAND), player, player.world); }
			 * if((oldValue&1)==0&&(message.value&1)!=0){
			 * ((ItemUsable)player.getHeldItem(InteractionHand.MAIN_HAND).getItem()).
			 * endUse(player.getHeldItem(InteractionHand.MAIN_HAND), player, player.world); } }
			 */
			/*
			...
			if (!stack.isEmpty() && stack.getItem() instanceof ItemUsable && oldState != (message.value & 3)
				...
				if ((oldState & 2) < (message.value & 2)) {
					...
					cap.stateDo(player, stack, InteractionHand.MAIN_HAND, stateOverride);
