...
public class CommandGiveWeapon extends CommandBase {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		...
		try {
			ServerPlayer ServerPlayer = args.length > 1 ? getPlayer(server, sender, args[1])
			...
			if (giveNew)
				...
				item = ServerPlayer.getHeldItemMainhand();
			boolean remove = args[0].equals("-r");
			CompoundTag attributes = item.getTagCompound().getCompoundTag("Attributes");
			...
			if (giveNew) {
				ItemHandlerHelper.giveItemToPlayer(ServerPlayer, item);
			} else
				item.getCapability(TF2weapons.WEAPONS_DATA_CAP, null).cached = false;
			/*
			 * Chunk
			 * chunk=ServerPlayer.world.getChunkFromBlockCoords(ServerPlayer.getPosition
			...
			// entityitem.func_145797_a(ServerPlayer.getCommandSenderName());
			// func_152373_a(p_71515_1_, this, "commands.giveweapon.success",
			// new Object[] {item.func_151000_E(),
			// ServerPlayer.getCommandSenderName()});
		} catch (Exception e) {
			throw new WrongUsageException(getUsage(sender), new Object[0]);
		}
	}
