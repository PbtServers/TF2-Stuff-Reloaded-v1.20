		...
		if (amount > 0) {
			...
			if (stack.getCount() > 1) {
				...
				if (living instanceof Player)
					remain = TF2Util.pickAmmo(remain, (Player) living, true);
				if (!remain.isEmpty())
			...
			return left;
			/*
			 * if (stack.getCount() <= 0 && living instanceof Player) { if
