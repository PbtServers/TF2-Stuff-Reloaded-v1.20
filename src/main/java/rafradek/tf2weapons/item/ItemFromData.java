...
public class ItemFromData extends Item implements IItemOverlay {
	...
	public ItemFromData() {
		...
		this.addPropertyOverride(new ResourceLocation("team"),
				(ItemStack stack, @Nullable Level world, @Nullable LivingEntity entityIn) -> {
					if (entityIn != null) {
	...
	public static ItemStack getNewStack(WeaponData type) {
		...
		tag.setTag("Attributes", new CompoundTag());
	...
	public static ItemStack getRandomWeapon(ItemStack stack, Random rand) {
		...
		if (!result.hasTagCompound()) result.setTagCompound(new CompoundTag());
	...
	public String getItemStackDisplayName(ItemStack stack) {
		...
		if (stack.getTagCompound().getBoolean("Australium"))
			name = ChatFormatting.GOLD + "Australium " + name;
		if (stack.getTagCompound().getBoolean("Valve"))
			name = ChatFormatting.DARK_PURPLE + "Valve " + name;
		return name;
	}
	...
	public void addInformation(ItemStack stack, Level world, List<String> tooltip, TooltipFlag advanced) {
		...
		if (stack.hasTagCompound()) {
			CompoundTag attributeList = stack.getTagCompound().getCompoundTag("Attributes");
			...
			while (iterator.hasNext()) {
				...
				if (tag instanceof FloatTag) {
					FloatTag tagFloat = (FloatTag) tag;
	...
	public void setRage(ItemStack stack, LivingEntity living, float value) {
		WeaponsCapability.get(living).setRage(this.getRageType(stack, living),
				Mth.clamp(value, 0f, this.getMaxRage(stack, living)));
	}
	...
	public ItemStack searchForAmmo(LivingEntity owner, ItemStack stack) {
		...
		for (int i = 0; i < ((Player) owner).inventory.mainInventory.size(); i++) {
			ItemStack stackInv = ((Player) owner).inventory.mainInventory.get(i);
