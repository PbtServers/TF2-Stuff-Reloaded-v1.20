...
public class TF2CraftingManager {
	...
	private final Map<IRecipe, Predicate<Player>> recipeConditions = new HashMap<>();
	...
	public TF2CraftingManager() {
		...
		banner.getOrCreateSubCompound("BlockEntityTag").setTag("Patterns", new ListTag());
		banner.setCount(2);
		...
		bannern.getOrCreateSubCompound("BlockEntityTag").setTag("Patterns", new ListTag());
		bannern.setCount(4);
	...
	public void addRecipeCondition(IRecipe recipe, Predicate<Player> predicate) {
	 ...
	 */
	@Nullable
	public ItemStack findMatchingRecipe(InventoryCrafting craftMatrix, Level world, Player player) {
	...
	public NonNullList<ItemStack> getRemainingItems(InventoryCrafting craftMatrix, Level world) {
		for (IRecipe irecipe : this.recipes)
