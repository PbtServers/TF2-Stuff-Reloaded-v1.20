...
import net.minecraft.world.level.block.entity.BlockEntityLockable;
...
public abstract class TileEntityAbstractAmmoFurnace extends TileEntityLockable implements ISidedInventory {
	...
	protected TileEntityAbstractAmmoFurnace(BlockEntityType<?> type, BlockPos pos, BlockState state) {
	...
	public void tick() {
		...
		if (isBurning() != burning) {
			flag1 = true;
			BlockAmmoFurnace.setState(burning, this.level, this.worldPosition);
		}
		if (flag1)
			this.setChanged();
	}
	...
	public boolean isUsableByPlayer(Player player) {
		return this.level.getBlockEntity(this.worldPosition) != this ? false
				: player.getDistanceSq(this.worldPosition.getX() + 0.5D, this.worldPosition.getY() + 0.5D, this.worldPosition.getZ() + 0.5D) <= 64.0D;
	}
