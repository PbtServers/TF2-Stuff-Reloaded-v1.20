...
public class GuiMercenary extends GuiMerchant {
	...
	public GuiMercenary(InventoryPlayer inv, EntityTF2Character mercenary, Level world) {
		super(inv, mercenary, world);
		this.mercenary = mercenary;
		this.inv = inv;
		inventorySlots = new ContainerMercenary(Minecraft.getMinecraft().player, mercenary, world);
		xSize += 54;
		// merInv=mercenary.getCapability(ForgeCapabilities.ITEM_HANDLER,
		// null);
	}
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		/*
		 * MerchantOffers merchantrecipelist = this.m.getRecipes(this.mc.player);
		 * 
		 * if (merchantrecipelist != null && !merchantrecipelist.isEmpty()) { int k =
		 * this.selectedMerchantOffer;
		 * 
		 * if (k < 0 || k >= merchantrecipelist.size()) { return; }
		 * 
		 * MerchantOffer merchantrecipe = (MerchantOffer)merchantrecipelist.get(k);
		 * 
