...
public class TF2EventsClient {
	...
	public static Vec3 armLpos;
	public static Vec3 armRpos;
	public static Vec3 armLang;
	public static Vec3 armRang;
		...
		if (minecraft.currentScreen == null) {
			...
			if (minecraft.gameSettings.keyBindJump.isPressed()) {
				...
				if (!minecraft.player.onGround && WeaponsCapability.get(minecraft.player).airJumps < WeaponsCapability
					...
					Vec3 moveDir = TF2Util.getMovementVector(minecraft.player);
					...
					if (minecraft.player.getHeldItemMainhand().getItem() instanceof ItemGrapplingHook) {
						WeaponsCapability.get(minecraft.player).setPrimaryCooldown(InteractionHand.MAIN_HAND,
			...
			if (ClientProxy.backpackitem.isPressed()) {
				...
				if (!backpack.isEmpty() && !((ItemBackpack) backpack.getItem())
					...
					minecraft.player.connection.sendPacket(new ServerboundSetCarriedItemPacket(8));
	...
	private static String getWittyComment() {
		String[] astring = new String[] { "Shiny numbers!", "Am I not running fast enough? :(",
				...
				"Hello Level", "40% better than a crash report.", "Now with extra numbers", "Now with less numbers",
		...
		if (changed && minecraft.currentScreen == null) {
			...
			if (item != null && item.getItem() instanceof ItemUsable
				...
				if ((oldState & 2) < (state & 2)) {
					// cap.setSecondaryCooldown(InteractionHand.OFF_HAND, ((ItemUsable)
					// item.getItem()).getAltFiringSpeed(item, player) / 2);
					cap.stateDo(player, item, InteractionHand.MAIN_HAND, stateOverride);
				...
				if ((oldState & 1) < (state & 1)) {
					// cap.setPrimaryCooldown(InteractionHand.OFF_HAND, ((ItemUsable)
	...
	public void clientTickEnd(TickEvent.ClientTickEvent event) {
		...
		if (event.phase == TickEvent.Phase.END) {
			...
			Iterator<LivingEntity> soundIterator = ClientProxy.soundsToStart.keySet().iterator();
			while (soundIterator.hasNext()) {
				LivingEntity living = soundIterator.next();
	 ...
	 * ((LivingEntity) event.getEntity()).getRNG().nextInt(); } }
	 *
	...
	public static int getActionType(boolean attackKeyDown, boolean altAttackKeyDown) {
		...
		if (Minecraft.getMinecraft().objectMouseOver != null
				&& Minecraft.getMinecraft().objectMouseOver.typeOfHit == HitResult.Type.BLOCK) {
	...
	public void getFov(FOVUpdateEvent event) {
		if (event.getEntity().getHeldItem(InteractionHand.MAIN_HAND) != null
				&& event.getEntity().getHeldItem(InteractionHand.MAIN_HAND).getItem() instanceof ItemUsable)
			if (event.getEntity().getHeldItem(InteractionHand.MAIN_HAND).getItem() instanceof ItemSniperRifle
	...
	public void renderOverlay(RenderGameOverlayEvent.Pre event) {
		...
		if (event.getType() == ElementType.HOTBAR) {
			...
			if (!held.isEmpty() && held.getItem() instanceof IItemOverlay) {
				...
				((IItemOverlay) held.getItem()).drawOverlay(held, player, Tesselator.getInstance(),
						Tesselator.getInstance().getBuffer(), event.getResolution());
			}
			...
			if (healTarget != null && healTarget instanceof LivingEntity) {
				LivingEntity living = (LivingEntity) healTarget;
	...
	public static void renderBeam(LivingEntity ent, float partialTicks, float size) {
			...
			if (TF2Util.isOnSameTeam(Minecraft.getMinecraft().player, event.getEntityPlayer())) {
				...
				if (WeaponsCapability.get(event.getEntityPlayer()).getDisguiseType().startsWith("M:")) {
					if (event.getEntityPlayer().getCapability(TF2weapons.WEAPONS_CAP, null).entityDisguise != null) {
						event.setDisplayname(ChatFormatting.RESET
								+ event.getEntityPlayer().getCapability(TF2weapons.WEAPONS_CAP, null).entityDisguise
										.getDisplayName().getFormattedText());
					} else
						event.setDisplayname(ChatFormatting.RESET + I18n.format("entity." + username + ".name"));
					...
					Player player = Minecraft.getMinecraft().world.getPlayers(Player.class,
	...
	public void renderLivingEntity(RenderLivingEvent.Pre<LivingEntity> event) {
		if (!event.getEntity().isEntityAlive())
			return;

		ClientProxy.renderCritGlow = 0;

		if (!(event.getEntity() instanceof Player || event.getEntity() instanceof EntityTF2Character))
			return;
		...
		if (event.getEntity().hasCapability(TF2weapons.INVENTORY_CAP, null)) {
			...
			for (int i = 0; i < cap.getSizeInventory(); i++) {
				...
				if (hat.getItem() instanceof ItemFromData
					...
					cap.origHead = event.getEntity().getItemStackFromSlot(EquipmentSlot.HEAD);
					if (event.getEntity() instanceof Player)
						((Player) event.getEntity()).inventory.armorInventory.set(3, ItemStack.EMPTY);
					else
						event.getEntity().setItemStackToSlot(EquipmentSlot.HEAD, ItemStack.EMPTY);
					break;
				}
			}
		}
		...
		if (visTick > 0) {
			...
			if (visTick >= 20) {
				...
				&& event.getEntity().getHeldItem(InteractionHand.MAIN_HAND) != null
				&& event.getEntity().getHeldItem(InteractionHand.MAIN_HAND).getItem() instanceof ItemUsable
				&& !(event.getEntity().getHeldItem(InteractionHand.MAIN_HAND).getItem() instanceof ItemMeleeWeapon)
		...
		if (event.getRenderer() != ClientProxy.disguiseRender && event.getRenderer() != ClientProxy.disguiseRenderPlayer
			...
			RenderLivingBase<LivingEntity> render = null;
			...
			if (seeDisguise && disguiseType.startsWith("M:")) {
				...
				LivingEntity entToRender = event.getEntity().getCapability(TF2weapons.WEAPONS_CAP,
						null).entityDisguise;
				if (entToRender == null || !EntityList.getKey(entToRender).equals(new ResourceLocation(mobType))) {
					entToRender = event.getEntity().getCapability(TF2weapons.WEAPONS_CAP,
							null).entityDisguise = (LivingEntity) EntityList
				...
				if (seeDisguise && disguiseType.startsWith("P:")) {
					Player player2 = event.getEntity().world.getPlayerEntityByName(disguiseType.substring(2));
			...
			if (render != null) {
				...
				if (origStack != null) {
					((Player) event.getEntity()).inventory = origStack;
				}
				if (mainhanditem != null)
					((Player) event.getEntity()).inventory.setInventorySlotContents(
							((Player) event.getEntity()).inventory.currentItem, mainhanditem);
			}
		}
	}

	public static boolean usesForceTexture(LivingEntity living) {
		return living instanceof Player && TF2PlayerCapability.get((Player) living).isForceClassTexture()
				&& WeaponsCapability.get(living).getUsedToken() >= 0;
	}

	@SubscribeEvent
	public void renderLivingPostEntity(RenderLivingEvent.Post<LivingEntity> event) {
		if (event.getRenderer().getRenderManager().isDebugBoundingBox() && !event.getEntity().isInvisible()
			...
			GlStateManager.disableBlend();
			AABB head = TF2Util.getHead(event.getEntity()).offset(-event.getEntity().posX,
		...
		if (event.getEntity().hasCapability(TF2weapons.INVENTORY_CAP, null)) {
			...
			if (!cap.origHead.isEmpty()) {
				if (event.getEntity() instanceof Player)
					((Player) event.getEntity()).inventory.armorInventory.set(3, cap.origHead);
				else
					event.getEntity().setItemStackToSlot(EquipmentSlot.HEAD, cap.origHead);
				cap.origHead = ItemStack.EMPTY;
			}
		}
		MobEffectInstance bullet = event.getEntity().getActivePotionEffect(TF2weapons.shieldBullet);
		MobEffectInstance explosive = event.getEntity().getActivePotionEffect(TF2weapons.shieldExplosive);
		MobEffectInstance fire = event.getEntity().getActivePotionEffect(TF2weapons.shieldFire);
		if (bullet != null || explosive != null || fire != null) {
			...
			if (hasShield) {
				...
				ClientRender.renderAABBTexture(
						new AABB(-width, 0, -width, width, event.getEntity().height + 0.25, width), 0, 0, 1,
						1);
				GlStateManager.color(1f, 1f, 1f, 1f);
				GlStateManager.popMatrix();
			}
