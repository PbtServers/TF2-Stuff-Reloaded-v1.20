...
public class TF2Attribute {
	...
	public static float addValue(float value, CompoundTag attributelist, String effect) {
	...
	public static float getModifierGlobal(String effect, float initial, LivingEntity entity) {
		float value = initial;
	...
	public String getTranslatedString(float value, boolean withColor) {
		...
		if (withColor) {
			ChatFormatting color = this.state == State.POSITIVE ? ChatFormatting.AQUA
					: (this.state == State.NEGATIVE ? ChatFormatting.RED : ChatFormatting.WHITE);
	...
	public static void upgradeItemStack(ItemStack stack, int value, Random rand) {
		...
		if (list.size() > 0) {
			int i = 0;
			CompoundTag tag = stack.getTagCompound().getCompoundTag("Attributes");
