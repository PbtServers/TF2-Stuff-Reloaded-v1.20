...
public class WeaponsCapability implements ICapabilityProvider, INBTSerializable<CompoundTag> {
	...
	private static final EntityDataAccessor<Boolean> CHARGING = new EntityDataAccessor<>(11, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<String> DISGUISE_TYPE = new EntityDataAccessor<>(7, EntityDataSerializers.STRING);
	private static final EntityDataAccessor<Boolean> DISGUISED = new EntityDataAccessor<>(8, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> INVIS = new EntityDataAccessor<>(9, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> FEIGN = new EntityDataAccessor<>(10, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Integer> CRIT_TIME = new EntityDataAccessor<>(0, EntityDataSerializers.VARINT);
	private static final EntityDataAccessor<Integer> HEADS = new EntityDataAccessor<>(1, EntityDataSerializers.VARINT);
	private static final EntityDataAccessor<Integer> HEAL_TARGET = new EntityDataAccessor<>(2, EntityDataSerializers.VARINT);
	private static final EntityDataAccessor<Integer> METAL = new EntityDataAccessor<>(3, EntityDataSerializers.VARINT);
	private static final EntityDataAccessor<Integer> TOKEN_USED = new EntityDataAccessor<>(12, EntityDataSerializers.VARINT);
	private static final EntityDataAccessor<Byte> CAN_FIRE = new EntityDataAccessor<>(13, EntityDataSerializers.BYTE);
	private static final EntityDataAccessor<Float> UBER_VIEW = new EntityDataAccessor<>(14, EntityDataSerializers.FLOAT);
	private static final EntityDataAccessor<Boolean> GRAPPLING = new EntityDataAccessor<>(15, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Boolean> GRAPPLED = new EntityDataAccessor<>(16, EntityDataSerializers.BOOLEAN);
	 ...
	 * headshotsRow; public int sapperTime; public LivingEntity
	 * buildingOwnerKill;
	 */
	...
	public boolean hasMetal(int metal) {
		boolean hasIngot = this.owner instanceof Player && TF2Util.hasEnoughItem(
				((Player) this.owner).inventory, stackL -> stackL.getItem() == Items.IRON_INGOT,
				Mth.ceil((metal - this.getMetal()) / 50f));
	...
	public int consumeMetal(int metal, boolean allowPartial) {
		...
		if (this.owner instanceof Player) {
			...
			while (((allowPartial && this.getMetal() == 0) || (!allowPartial && this.getMetal() < metal))
					&& ((Player) this.owner).inventory.hasItemStack(ingot)) {
				((Player) this.owner).inventory.clearMatchingItems(Items.IRON_INGOT, 0, 1, null);
	...
	public void setMetal(int metal) {
		this.dataManager.set(METAL, Mth.clamp(metal, 0, this.getMaxMetal()));
	}

	public void giveMetal(int metal) {
		this.dataManager.set(METAL, Mth.clamp(this.getMetal() + metal, 0, this.getMaxMetal()));
	}
	...
	public void addEffectCooldown(String name, int time) {
		...
		if (this.owner instanceof ServerPlayer)
			TF2weapons.network.sendTo(new TF2Message.EffectCooldownMessage(name, time), (ServerPlayer) this.owner);
	}
	...
	public void onChangeValue(EntityDataAccessor<?> param, Object newValue) {
	...
	public boolean isUsingParachute() {
		ItemStack stack = this.owner.getItemStackFromSlot(EquipmentSlot.CHEST);
	...
	public void tick() {
		...
		if (!this.owner.world.isRemote && TF2ConfigVars.headCountDrop && this.dataManager.get(HEADS) > 0
				&& collectedHeadsTime < this.owner.ticksExisted
						- Math.max(100, 2000 - Mth.log2(this.dataManager.get(HEADS)) * 300)) {
			this.dataManager.set(HEADS, this.dataManager.get(HEADS) - 1);
			collectedHeadsTime = this.owner.ticksExisted;
		...
		if (!this.owner.world.isRemote && this.owner instanceof Player && this.owner.ticksExisted % 20 == 0) {
			this.maxmetal = TF2Attribute.getModifier("Max Metal",
					TF2Util.getBestItem(((Player) this.owner).inventory, (stack1,
		...
		if (this.grapplingHook != null) {
			...
			if (!this.grapplingHook.isEntityAlive()) {
				...
				// false, 0f, false).get(0).typeOfHit == HitResult.Type.BLOCK)
				// this.setGrapplingHook(null);
				// else {
				Vec3 motionVec = new Vec3(this.grapplingHook.posX - this.owner.posX,
		...
		for (InteractionHand hand : InteractionHand.values()) {
			...
			boolean isUseableWeapon = !stack.isEmpty() && stack.getItem() instanceof ItemUsable
					&& (hand == InteractionHand.MAIN_HAND
							|| (hand == InteractionHand.OFF_HAND && !hadItem
			...
			if (isUseableWeapon) {
				...
				if ((!owner.world.isRemote || owner != Minecraft.getMinecraft().player)
					...
					if (((state & 4) != 0 || emptyMag || continueReload) && stackcap.active == 2
						...
						if (!owner.world.isRemote && owner instanceof ServerPlayer)
			...
			if (this.stackActive.get(hand) != null) {
				if (owner.world.isRemote && !(owner instanceof Player) && isUseableWeapon
		...
		MobEffectInstance charging = owner.getActivePotionEffect(TF2weapons.charging);

		/*
		 * if (owner instanceof Player && owner.onGround && owner.motionY < 0) {
		...
		if (this.isExpJump() && this.canExpJump()) {
			boolean enchanted = TF2ConfigVars.enchantedExplosion && !owner.isElytraFlying() && !this.isUsingParachute()
					&& !(owner instanceof Player && ((Player) owner).capabilities.isFlying)
					&& !owner.getItemStackFromSlot(EquipmentSlot.FEET).getItem().getRegistryName().toString()
							.equals("tconstruct:slime_boots");
			if (owner.onGround) {
				BlockPos pos = owner.getPosition().down();
				BlockState block = owner.world.getBlockState(pos);
			...
			if (enchanted) {
				...
				/*
				 * if(!owner.world.isRemote && owner instanceof Player &&
				...
				if ((!owner.world.isRemote || owner instanceof Player)
						&& !(TF2weapons.squakeLoaded && owner instanceof Player)) {

					double speed = Math.sqrt(owner.motionX * owner.motionX + owner.motionZ * owner.motionZ);

					Vec3 moveDir = TF2Util.getMovementVector(owner);
		...
		if (this.isExpJump() || charging != null) {
			Vec3 vec = new Vec3(owner.motionX, owner.motionY, owner.motionZ);
			Vec3 vec2 = new Vec3(owner.motionX, 0, owner.motionZ);
			if (owner.onGround)
				vec = vec2;
			double motion = vec.lengthVector();
			// double motiona = vec2.lengthVector();
			if (motion >= 0.375 && (TF2ConfigVars.allowTrimp == 2 || (TF2ConfigVars.allowTrimp == 1 && Direction
					.getFacingFromVector((float) vec.x, (float) vec.y, (float) vec.z) != Direction.DOWN))) {
				vec2 = vec2.normalize();
				Vec3 entPos = owner.getPositionVector();
				Vec3 vecOff = vec.add(vec2.scale(owner.width * 1.5));
				HitResult rayTrace = owner.world.rayTraceBlocks(entPos, entPos.add(vecOff));
				...
				if (owner.world.isBlockFullCube(pos.down())) {
					Vec3 step = TF2Util.getHeightVec(owner.world, pos);
					Vec3 normal = step.normalize();
					if (normal.y != 1D && normal.dotProduct(vecOff) < 0 && !(charging != null && normal.y > 0.75)) {
						double backoff = vec.dotProduct(normal);
						vec = new Vec3(vec.x - normal.x * backoff, vec.y - normal.y * backoff,
		...
		if (!owner.world.isRemote && this.disguiseTicks > 0) {
			...
			if (++this.disguiseTicks >= 40) {
				...
				((ServerLevel) owner.world).spawnParticle(EnumParticleTypes.SMOKE_LARGE, owner.posX, owner.posY,
		...
		if (!owner.world.isRemote && charging != null) {
			...
			Vec3 start = owner.getPositionVector().addVector(0, owner.height / 2, 0);
			Vec3 end = start.addVector(-Mth.sin(owner.rotationYaw / 180.0F * (float) Math.PI) * 0.7, 0,
					Mth.cos(owner.rotationYaw / 180.0F * (float) Math.PI) * 0.7);
			HitResult result = TF2Util
		...
		if (owner.world.isRemote && WeaponsCapability.get(owner).isDisguised() != this.lastDisgused) {
			...
			if (owner instanceof Player)
				((Player) owner).refreshDisplayName();
		}
		...
		if (owner.world.isRemote && !disguisetype.equals(this.lastDisguiseValue)) {
			...
			if (this.getDisguiseType().startsWith("P:")) {
				...
				Player playerDisguise = this.owner.world.getPlayerEntityByName(getDisguiseType().substring(2));
...
if (typeIn == Type.SKIN) {
		...
		if (owner.world.isRemote && owner != ClientProxy.getLocalPlayer()) {
			...
			Iterator<MobEffectInstance> iterator = owner.getActivePotionEffects().iterator();
		...
		if (!owner.world.isRemote && owner.fallDistance > 0
				&& owner.getItemStackFromSlot(EquipmentSlot.FEET).getItem() == TF2weapons.itemMantreads) {
			TF2Util.stomp(owner);
		}
		if (this.isInvisible()) {
			...
			if (!visible && !feign) {
				List<Entity> closeEntities = owner.world.getEntitiesInAABBexcluding(owner,
						owner.getEntityBoundingBox().grow(1, 2, 1), input -> input instanceof LivingEntity && !TF2Util.isOnSameTeam(owner, input));
	...
	public boolean shouldShoot(LivingEntity player, int state, InteractionHand hand, int actualState) {
		return (!(!player.world.isRemote && player instanceof Player
				&& this.predictionList[hand.ordinal() + (state == 1 ? 0 : 2)].isEmpty()))
				&& !((player.world.isRemote || !(player instanceof Player)) && (actualState & state) != state);
	}
	...
	public void stateDo(LivingEntity player, ItemStack stack, InteractionHand hand, int state) {
		...
		while (stackcap.fire1Cool <= 0 && shouldShoot(player, 1, hand, state)) {
			...
			if (!player.world.isRemote && player instanceof Player) {
			...
			if (message == null) {
				...
				this.mainHand = message.hand == InteractionHand.MAIN_HAND;
			...
			if (ItemUsable.isDoubleWielding(player)) {
				if (hand == InteractionHand.MAIN_HAND)
					this.setPrimaryCooldown(InteractionHand.OFF_HAND,
							Math.max(this.getPrimaryCooldown(InteractionHand.OFF_HAND), fireRate / 2 - 50));
				else if (hand == InteractionHand.OFF_HAND)
					this.setPrimaryCooldown(InteractionHand.MAIN_HAND,
							Math.max(this.getPrimaryCooldown(InteractionHand.MAIN_HAND), fireRate / 2 - 50));
			}
		...
		while (stackcap.fire2Cool <= 0 && shouldShoot(player, 2, hand, state)) {
			...
			if (player.world.isRemote && player == ClientProxy.getLocalPlayer())
				TF2weapons.network.sendToServer(new TF2Message.PredictionMessage(player.posX, player.posY, player.posZ,
						player.rotationPitch, player.rotationYawHead, 2, InteractionHand.MAIN_HAND));
	...
	public CompoundTag serializeNBT() {
		...
		CompoundTag cld = new CompoundTag();
	...
	public void deserializeNBT(CompoundTag nbt) {
		...
		CompoundTag cld = nbt.getCompoundTag("Cooldowns");
	...
	public int getMaxAirJumps() {
		...
		if (this.owner instanceof EntityScout)
			amount += 1;
		else if (this.owner instanceof Player && (this.getUsedToken() == 0 || (ItemToken.allowUse(owner, TF2Class.SCOUT)
				&& owner.getItemStackFromSlot(EquipmentSlot.FEET).getItem() == TF2weapons.itemScoutBoots))) {
			amount += 1;
		}
	...
	public int getMinimalCooldown() {
		if (!this.owner.world.isRemote && this.owner instanceof Player)
	...
	public int getPrimaryCooldown() {
		return Math.max(this.getPrimaryCooldown(InteractionHand.MAIN_HAND), this.getPrimaryCooldown(InteractionHand.OFF_HAND));
	}

	public void setPrimaryCooldown(int fire1Cool) {
		this.setPrimaryCooldown(InteractionHand.MAIN_HAND, fire1Cool);
		this.setPrimaryCooldown(InteractionHand.OFF_HAND, fire1Cool);
	}

	public int getPrimaryCooldown(InteractionHand hand) {
	...
	public void setPrimaryCooldown(InteractionHand hand, int fire1Cool) {
	...
	public int getSecondaryCooldown() {
		return Math.max(this.getSecondaryCooldown(InteractionHand.MAIN_HAND), this.getSecondaryCooldown(InteractionHand.OFF_HAND));
	}

	public void setSecondaryCooldown(int fire1Cool) {
		this.setSecondaryCooldown(InteractionHand.MAIN_HAND, fire1Cool);
		this.setSecondaryCooldown(InteractionHand.OFF_HAND, fire1Cool);
	}

	public int getSecondaryCooldown(InteractionHand hand) {
		if (this.owner.getHeldItem(hand).getItem() instanceof ItemUsable)
	...
	public void setSecondaryCooldown(InteractionHand hand, int fire1Cool) {
		if (this.owner.getHeldItem(hand).hasCapability(TF2weapons.WEAPONS_DATA_CAP, null))
	...
	public void setActiveHand(InteractionHand hand, ItemStack stack) {
		this.stackActive.put(hand, stack);
		stack.getCapability(TF2weapons.WEAPONS_DATA_CAP, null).active = 2;
	...
	public void setInactiveHand(InteractionHand hand, ItemStack stack) {
	...
	public void disguise() {
		...
		if (this.invisTicks < 20 && this.owner.Level instanceof ServerLevel)
			((ServerLevel) this.owner.world).spawnParticle(EnumParticleTypes.SMOKE_LARGE, this.owner.posX,
	...
	static {
		...
		for (RageType type : RageType.values()) {
			RAGE.put(type, new EntityDataAccessor<>(127 - type.ordinal() * 2, EntityDataSerializers.FLOAT));
			RAGE_ACTIVE.put(type, new EntityDataAccessor<>(126 - type.ordinal() * 2, EntityDataSerializers.BOOLEAN));
		}
	}
