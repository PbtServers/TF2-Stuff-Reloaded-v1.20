...
public class EntityNBTCondition implements LootCondition {
	...
	public EntityNBTCondition(CompoundTag test, boolean negate) {
		this.negate = negate;
		this.tagValue = test;
	}
	...
	public boolean testCondition(Random rand, LootContext context) {
		...
		CompoundTag tag = test.writeToNBT(new CompoundTag());
	...
	public static class Serializer extends LootCondition.Serializer<EntityNBTCondition> {
		...
		public EntityNBTCondition deserialize(JsonObject json, JsonDeserializationContext context) {
			try {
				return new EntityNBTCondition(JsonToNBT.getTagFromJson(GsonHelper.getString(json, "test", "{}")),
						GsonHelper.getBoolean(json, "negate", false));
			} catch (NBTException e) {
				e.printStackTrace();
			}
			return new EntityNBTCondition(new CompoundTag(), false);
		}
	}
}
