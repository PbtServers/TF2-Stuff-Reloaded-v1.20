...
public class EntityAIAmbush extends EntityAIBase {
	...
	public void updateTask() {
		...
		if (!this.host.getWepCapability().isDisguised()) {
			if (this.host.getAttackTarget() != null && this.host.getAttackTarget() instanceof Player)
		...
		if (dist > 16 || !TF2Util.lookingAtFast(target, 105, this.host.posX, this.host.posY, this.host.posZ)) {
			float x = -Mth.sin(target.rotationYaw / 180.0F * (float) Math.PI);
			float z = Mth.cos(target.rotationYaw / 180.0F * (float) Math.PI);
			Vec3 pos = new Vec3(target.posX - x * 2, target.posY, target.posZ - z * 2);
			host.getNavigator().tryMoveToXYZ(target.posX - x * 2, target.posY, target.posZ - z * 2, 1);
		} else {
			float x = -Mth.sin(target.rotationYaw / 180.0F * (float) Math.PI);
			float z = Mth.cos(target.rotationYaw / 180.0F * (float) Math.PI);
			Vec3 pos = new Vec3(x, 0, z).crossProduct(new Vec3(0, 1, 0));
