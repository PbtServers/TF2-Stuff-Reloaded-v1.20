...
public class EntityAIUseRangedWeapon extends EntityAIBase {
	...
	public Vec3 velTarget;
	public Vec3 posTarget;
	...
	public boolean shouldExecute() {
		...
		if (LivingEntity == null)
			...
			if (this.attackTarget != LivingEntity)
				this.lastMotionY = LivingEntity.motionY;
	...
	public void resetTask() {
		if ((this.entityHost.getHeldItem(InteractionHand.MAIN_HAND).getItem() instanceof ItemWeapon)
				&& this.entityHost.getCapability(TF2weapons.WEAPONS_CAP, null).state != 0) {
			pressed = false;
			((ItemWeapon) this.entityHost.getHeldItem(InteractionHand.MAIN_HAND).getItem()).endUse(
					this.entityHost.getHeldItem(InteractionHand.MAIN_HAND), this.entityHost, this.entityHost.world,
					this.entityHost.getCapability(TF2weapons.WEAPONS_CAP, null).state, 0);
			this.entityHost.getCapability(TF2weapons.WEAPONS_CAP, null).state = 0;
	...
	public void updateTask() {
		...
		if (this.projSpeed > 0) {
			...
			int ticksToReach = Mth.ceil(dist / this.projSpeed);
			...
			if (this.fireAtFeet == -1 || this.entityHost.world.rayTraceBlocks(new Vec3(this.entityHost.posX,
					this.entityHost.posY + this.entityHost.getEyeHeight(), this.entityHost.posZ),
					new Vec3(lookX, lookY, lookZ), false, true, false) != null)
			...
			if (!this.attackTarget.isInWater()) {
				...
				if (!this.attackTarget.onGround && this.attackTarget.motionY < 0)
					yFall += Mth.clamp(this.lastMotionY - this.attackTarget.motionY, 0, 0.1) * i;
				/*
			...
			HitResult mop = this.entityHost.world.rayTraceBlocks(this.attackTarget.getPositionVector(),
			...
			if (this.fireAtFeet > 0 && this.entityHost.world.rayTraceBlocks(
					new Vec3(this.entityHost.posX, this.entityHost.posY + this.entityHost.getEyeHeight(),
							this.entityHost.posZ),
					new Vec3(lookX, this.attackTarget.posY, lookZ), false, true, false) == null) {
			...
			/*
			 * HitResult ray = this.entityHost.rayTrace(128d, 1f); if (ray != null)
			 * ((ServerLevel)this.entityHost.world).spawnParticle(EnumParticleTypes.CRIT,
		...
		boolean fire = stay && this.entityHost.getActivePotionEffect(TF2weapons.stun) == null && shouldFireProj
				...
				&& TF2Util.lookingAt(this.entityHost,
						(this.explosive && d0 < 16 ? 30 : 0) + 2
								+ Math.toDegrees(Mth.atan2(this.attackTarget.width / 2, dist)),
						lookX, lookY, lookZ);

		if (!this.reloading && (this.entityHost.world.getDifficulty() != Difficulty.HARD || !fire)
