...
public class EntityAIStickybomb extends EntityAIBase {
	...
	public void updateTask() {
		...
		if (d0 <= this.attackRangeSquared) {
			if (!pressed) {
				pressed = true;
				((ItemUsable) this.entityHost.getHeldItem(InteractionHand.MAIN_HAND).getItem()).startUse(
						this.entityHost.getHeldItem(InteractionHand.MAIN_HAND), this.entityHost, this.entityHost.world,
			...
			if (pressed) {
				((ItemUsable) this.entityHost.getHeldItem(InteractionHand.MAIN_HAND).getItem()).endUse(
						this.entityHost.getHeldItem(InteractionHand.MAIN_HAND), this.entityHost, this.entityHost.world,
						this.entityHost.getCapability(TF2weapons.WEAPONS_CAP, null).state, 0);
				this.entityHost.getCapability(TF2weapons.WEAPONS_CAP, null).state = 0;
				TF2Util.sendTracking(new TF2Message.ActionMessage(0, entityHost), entityHost);
