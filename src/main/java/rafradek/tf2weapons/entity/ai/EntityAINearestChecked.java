...
import rafradek.tf2weapons.entity.ai.EntityAINearestAttackableTarget.Sorter;
...
public class EntityAINearestChecked<T extends LivingEntity> extends EntityAITarget {
			...
			if (p_i1665_6_ != null && !p_i1665_6_.apply(target))
				...
				if (target instanceof LivingEntity) {
					...
					if (target instanceof Player && target.isInvisible()) {
						float f = ((Player) target).getArmorVisibility();
	...
	public boolean shouldExecute() {
		...
		if (((this.taskOwner.getAttackTarget() == null) && this.targetChoosen > 1) || this.targetChoosen > 5
			...
			List<? extends LivingEntity> list = this.taskOwner.world.getEntitiesWithinAABB(this.targetClass,
			...
			if ((team != null && team1 == team) && !(this.taskOwner instanceof EntityMedic))
				...
				if (this.taskOwner.getDistanceSq(entitylivingbase) > d0 * d0)
					...
					return !(entitylivingbase instanceof Player)
							|| !((Player) entitylivingbase).capabilities.disableDamage;
				}
			}
		}
	}
