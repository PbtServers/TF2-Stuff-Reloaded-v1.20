...
public class EntityAISetup extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		if (buildType > 0) {
			...
			/*
			 * this.engineer.setItemStackToSlot(EquipmentSlot.MAINHAND, new
			 * ItemStack(TF2weapons.itemBuildingBox, 1, 16 + buildType * 2 +
			 * this.engineer.getEntTeam()));
			 * this.engineer.getHeldItem(InteractionHand.MAIN_HAND).setTagCompound(new
			 * CompoundTag());
			 */
		}
	...
	public void updateTask() {
		if (this.buildType > 0) {
			...
			if (this.engineer.getNavigator().noPath()) {
				...
				Vec3 Vec3 = RandomPositionGenerator.findRandomTarget(this.engineer, size + 1, size);
				if (Vec3 != null) {
					AABB box = new AABB(Vec3.x - 0.5, Vec3.y, Vec3.z - 0.5, Vec3.x + 0.5,
							Vec3.y + 1, Vec3.z + 0.5);
					List<AABB> list = this.engineer.world.getCollisionBoxes(this.engineer, box);

					if (list.isEmpty() && !this.engineer.world.isMaterialInBB(box, Material.WATER)) {
						this.engineer.getNavigator().tryMoveToXYZ(Vec3.x, Vec3.y, Vec3.z, 1);
						this.target = Vec3;
					}
				}
				/*
				 * for(AABB entry:list){ System.out.println(entry); }
				 */
	...
	public EntityBuilding spawn() {
		...
		BlockState blockTarget = this.engineer.world.getBlockState(new BlockPos(target));
