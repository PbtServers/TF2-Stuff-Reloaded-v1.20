...
public class EntityScout extends EntityTF2Character {
	...
	public EntityScout(Level par1World) {
		super(par1World);
		if (this.attack != null) {
	...
	protected void jump() {
		 ...
		 */
		this.motionX = (-Mth.sin(this.rotationYaw / 180.0F * (float) Math.PI));
		this.motionZ = (Mth.cos(this.rotationYaw / 180.0F * (float) Math.PI));
		float f2 = (float) (Mth.sqrt(motionX * motionX + motionZ * motionZ)
	 ...
	 * this.getHeldItem(InteractionHand.MAIN_HAND); float f = (float)(this.getMaxHealth() -
	 * this.getHealth()) / (float)this.getMaxHealth(); int i = 4 +
	 * Mth.floor_float(f * 4.0F);
	 *
