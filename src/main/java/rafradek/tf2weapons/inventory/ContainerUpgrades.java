...
public class ContainerUpgrades extends Container {
	...
	public ContainerUpgrades(Player player, InventoryPlayer inventory, TileEntityUpgrades station,
	...
	public boolean enchantItem(Player player, int id) {
		...
		CompoundTag stacktag = stack.getTagCompound();
		...
		if (id >= 0) {
			...
			if (adding && currLevel < this.station.attributes.get(attrorig) && expPoints >= cost
				...
				CompoundTag tag = stacktag.getCompoundTag("Attributes");
