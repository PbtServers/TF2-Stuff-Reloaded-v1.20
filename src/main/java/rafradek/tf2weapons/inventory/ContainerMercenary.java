...
public class ContainerMercenary extends ContainerMerchant {
	...
	public ContainerMercenary(Player player, EntityTF2Character merc, Level world) {
		...
		if (this.mercenary.hasHeldInventory()) {
			for (int i = 0; i < 4; i++) {
				 ...
				 * this.mercenary.loadout.getStackInSlot(i)), (ServerPlayer) player);
				 */
			}
			{
				int i = 3;
		...
		for (int k = 0; k < 4; ++k) {
			...
			this.addSlotToContainer(new SlotItemHandler(
					merc.getCapability(ForgeCapabilities.ITEM_HANDLER, Direction.NORTH), 3 - k, 184,
					8 + k * 18) {
				/**
				...
				public ItemStack getStack() {
					if (entityequipmentslot == EquipmentSlot.HEAD && this.getItemHandler()
				...
				/*
				 * @Override public boolean canTakeStack(Player player) { return
				 * merc.getDropChance(); }
				 */
		...
		for (int i = 0; i < 4; i++) {
			...
			this.addSlotToContainer(
					new SlotItemHandler(merc.hasHeldInventory() ? merc.loadoutHeld : merc.loadout, i, 206, 8 + i * 18) {
						 ...
						 * @Nullable
						 * 
						 * @OnlyIn(Dist.CLIENT) public String getSlotTexture() { return
	...
	public ItemStack transferStackInSlot(Player player, int index) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			EquipmentSlot equip = Mob.getSlotForItemStack(itemstack);
			...
			if (index == 2) {
				...
				if (equip.getSlotType() == EquipmentSlot.Type.ARMOR) {
